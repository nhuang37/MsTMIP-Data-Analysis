...
def protected_mask(path_nc4, path_protected_shp, lats, lons):
	...
	for i in range(12): 
	    t = len(time[:])/12 #get time-length, lat length, lon length
	    ...
	    for coords in list_protected:
	    	...
	    	lai_monthly = LAI[np.arange(i,360,12),lat,lon]  #subset per month per lat lon, using the calculated mean_file
	...
	with open('cat_lai3g_ken_sur.txt', 'w') as outfile:
		outfile.write('# Array shape: {0}\n'.format(lai_cat.shape))
		for data_slice in lai_cat:
			np.savetxt(outfile, data_slice, fmt='%-7.2f')
			outfile.write('# New slice\n')
	# Write the array to disk
	with open('mkt_lai3g_ken_sur.txt', 'w') as outfile:
	...
	return lai_cat, lai_mkt, latbounds, lonbounds, 'LAI3G'
...
def main(spark,LAI3G_file, Region_shp):
	#get nc4 files
	mypath = '/Users/nhuang37/Desktop/NYU DS/Yr1 Summer/Data/LAI/Taylor Diagram/LAI3G_regrid.nc'
	...
	category_mask, stiple_mask, latbounds, lonbounds, title  = protected_mask(mypath, tanzania_shp, lats, lons)
	plot(category_mask, stiple_mask, title, lats, lons, latbounds, lonbounds)


if __name__ == '__main__':
	# Create the spark session object
	spark = SparkSession.builder.appName('LAI3G_stipple_ken').getOrCreate()

	# Get the data file from the command line
	LAI3G_file = sys.argv[1]

	# Get the model file from the command line
	Region_shp = sys.argv[2]

	# Call our main routine
	main(spark,LAI3G_file,Region_shp)
